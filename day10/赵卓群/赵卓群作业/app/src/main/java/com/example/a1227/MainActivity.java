...
import com.example.a1227.adpte.PoiListAdapter;
...
public class MainActivity extends AppCompatActivity implements OnGetPoiSearchResultListener,
    ...
    public void searchNearbyProcess(View v) {
        ...
        if (strLatitude.isEmpty() || strLongitude.isEmpty()) {
            Toast.makeText(MainActivity.this, "检索经纬度是必填参数", Toast.LENGTH_LONG).show();
            return;
        }

        if (strRadius.isEmpty()) {
            Toast.makeText(MainActivity.this, "检索半径是必填参数", Toast.LENGTH_LONG).show();
            return;
        }

        if (keyWorlds.isEmpty()) {
            Toast.makeText(MainActivity.this, "检索关键字是必填参数", Toast.LENGTH_LONG).show();
            return;
        }
